/* ═══ A7 초음파 국소화 현미경 · 애니메이션 ═══
   검증: /home/claude/work/a7.js · a7b.js  (Renaudin 2022 · Heiles 2022 세팅)
     ...
     · 깊이 10mm → F# 0.71 → 측방향 −6dB = 0.886λF# = 64.6 µm · σ_PSF = FWHM/2.355 = 27.4 µm
     ★ 계수는 **왕복(펄스-에코) 1차원 개구** 기준 0.886 하나로 통일한다.
       예전에는 W6 = 1.207λF#(편도)를 자·판정에, FWHM = 0.886λF#(왕복)를 그래프 σ 에 함께 써서
       **자가 그래프보다 36% 넓었다.** 그 결과 간격 55~88 µm 에서 그래프는 두 봉우리로 갈라지는데
       판정은 "하나로 뭉침" 을 표시했다(§7-1 #15). ULM 이 국소화하는 PSF 는 왕복이므로 0.886 이 맞다.
       17장의 1.02λF# 는 **원형 개구** 왕복 계수라 또 다른 기하이다.
...
const FWHM = 0.886*LAM*FNUM;             /* mm — 왕복 sinc² −6dB (1차원 개구) */
const W6   = FWHM;                       /* PSF 폭은 하나뿐 — 자·판정·면적 모두 이 값을 쓴다 */
