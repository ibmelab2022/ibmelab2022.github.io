...
관심 있는 학생은 자기소개서(1장 이내)와 성적표를 첨부하여 아래 이메일로 연락 주세요.`,
  ...
  research: [
    {
      ...
      bodyEn: `We investigate next-generation ultrasound localization microscopy (ULM) techniques to visualize microvascular networks beyond the diffraction limit. Our research integrates ultrafast ultrasound imaging, advanced beamforming, novel microbubble localization and tracking algorithms, motion correction, and quantitative vascular imaging to enable high-resolution visualization of microvascular structure and blood flow.`,
      bodyKr: `초음파 회절 한계를 극복하여 미세 혈관 구조를 영상화하는 초음파 국소화 현미경(Ultrasound Localization Microscopy, ULM) 기술을 연구합니다. 초고속 초음파 영상(Ultrafast Ultrasound Imaging), 고급 빔포밍(Advanced Beamforming), 새로운 미세기포 위치 추정 및 추적 알고리즘(Novel Microbubble Localization & Tracking Algorithms), 움직임 보정(Motion Correction), 정량적 혈관 영상(Quantitative Vascular Imaging) 기술을 통해 미세 혈관의 구조와 혈류를 정밀하게 영상화하는 차세대 초음파 의료영상 기술을 구현합니다.`,      
      image:    "",
    ...
    },
    {
      ...
      bodyEn: `We develop high-frequency acoustic imaging systems based on scanning acoustic microscopy (SAM) and ultrasound biomicroscopy (UBM). By integrating acoustic imaging, signal processing, AI-based image enhancement, and automated analysis, our technologies support both biomedical imaging and nondestructive inspection of semiconductor devices and advanced electronic packages.`,
      bodyKr: `동일한 고주파 초음파 원리를 공유하는 Scanning Acoustic Microscopy(SAM)와 Ultrasound Biomicroscopy(UBM) 기반의 고주파 음향 영상 시스템을 개발합니다. 해당 기술은 조직 및 미세 생체 구조의 의료영상과 반도체 소자 및 첨단 전자 패키지의 비파괴 검사에 모두 활용됩니다. 음향 영상, 신호처리, AI 기반 영상 향상 및 자동 분석 기술을 융합하여 의료와 산업 분야를 아우르는 고성능 초음파 영상 솔루션을 제공합니다.`,      
      image:       "images/research/sam_system.jpg",
    ...
    },
    {
      ...
      bodyEn: `We explore computational imaging techniques that improve image quality while reducing acquisition time and hardware complexity. Our research includes coded-mask imaging, compressed sensing, sparse reconstruction, inverse imaging, and advanced beamforming algorithms for efficient next-generation ultrasound imaging.`,
      bodyKr: `영상 품질 향상과 데이터 획득 시간 단축을 동시에 달성하기 위한 계산영상(Computational Imaging) 기술을 연구합니다. Coded-mask Imaging, Compressed Sensing, Sparse Reconstruction, 역문제(Inverse Imaging) 기반 영상 복원 및 고급 빔포밍 알고리즘을 활용하여 제한된 데이터에서도 고품질 영상을 복원하는 차세대 영상 재구성 기법을 제안합니다.`,      
      image:       "images/research/cus_computational.jpg",
    ...
    },
    {
      ...
      bodyEn: `We design FPGA-based imaging platforms for real-time ultrasound and acoustic imaging applications. Our work emphasizes high-speed data acquisition, real-time beamforming, hardware acceleration, and scalable system architectures to deliver high-performance embedded imaging systems.`,
      bodyKr: `실시간 초음파 및 음향 영상 시스템을 위한 FPGA 기반 고성능 이미징 플랫폼을 설계합니다. 고속 데이터 획득, 실시간 빔포밍, 하드웨어 가속 및 확장 가능한 시스템 아키텍처를 바탕으로 고성능 영상 알고리즘을 실시간으로 구현할 수 있는 임베디드 영상 시스템을 구축합니다.`,      
      image:       "images/research/fpga_boards.jpg",
      mediaTitle:  "In-house imaging hardware boards",
      mediaCredit: "J. Kim et al., IEEE–TMI, 2020",
    },
  ...
  /* ───────── ⑪ Resources (학습자료 · 연구장비) ─────────
     홈페이지 상단 "Resources" 탭의 내용입니다.

     · resourcesIntro : 탭 맨 위 안내 문구 (없애려면 "" 로 두세요)

     · studyMaterials : 학습자료 카드. 자료를 추가하려면 아래 { … }, 를 하나 더 넣으면 됩니다.
         title  한글 제목           en    영문 제목 (옆에 작게)
         meta   위쪽 작은 회색 글씨   href  링크 (폴더 경로 또는 주소)
         desc   설명 (**굵게** 표시 가능)   cta   버튼 문구

     · equipment : 연구장비 카드. 아래 [ ] 안에 { … }, 를 하나씩 넣으면 자동으로 나타납니다.
         비워두면 "Equipment · 연구장비" 소제목째로 안 보입니다.
         사진은 images/ 폴더에 올린 뒤 파일명을 image 에 적으세요.
         (사진이 없으면 image 줄을 통째로 지우면 됩니다)

         작성 예시 — equipment 의 [ ] 안에 이대로 넣고 내용만 바꾸세요:

             {
               name:  "Verasonics Vantage 256",
               ko:    "초음파 리서치 플랫폼",
               image: "images/equip_verasonics.jpg",
               specs: [
                 "256채널 병렬 송수신",
                 "샘플링 최대 62.5 MHz",
                 "MATLAB 기반 시퀀스 프로그래밍",
               ],
               note:  "평면파 · 합성개구 등 커스텀 송수신 실험에 사용합니다.",
             },
  ───────── */
  resourcesIntro: `IBME Lab 구성원을 위한 학습자료와 연구장비 정보입니다.
새로 합류한 학생은 아래 학습자료를 순서대로 따라가며 연구실 주제의 배경지식을 스스로 익힐 수 있습니다.`,
